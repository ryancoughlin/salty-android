...
/**
 * Wind conditions bar chart.
 * Matches iOS WindConditionsChart.
 */
@Composable
fun WindConditionsChart(
    ...
    val filtered = remember(forecasts, date) {
        forecasts.filter {
            it.time.atZone(ZoneId.systemDefault()).toLocalDate() == date
        }.sortedBy { it.time }
    }

    if (filtered.isEmpty()) return

    val maxSpeed = remember(filtered) {
        (filtered.maxOfOrNull { it.speed } ?: 10.0) * 1.33
    }
    ...
    Canvas(
        ...
        filtered.forEachIndexed { index, forecast ->
            ...
            val barHeight = ((forecast.speed / maxSpeed) * chartHeight).toFloat()
            val barTop = topPadding + chartHeight - barHeight
            val barColor = ChartColorScales.windColor(forecast.speed)

            val isNow = isClosestToNow(forecast.time, filtered.map { it.time }, now, date)
            if (isNow) {
                drawLine(
            ...
            drawContext.canvas.nativeCanvas.apply {
                ...
                drawText(
                    String.format("%.0f", forecast.speed),
                    x + barWidth / 2,
                    barTop - 8.dp.toPx(),
                    paint
                )
            ...
            drawContext.canvas.nativeCanvas.apply {
                ...
                val label = hourFormatter.format(forecast.time).lowercase()
