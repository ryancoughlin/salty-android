...
private const val TAG = "SatelliteTrackLayer"
...
/**
 * Renders satellite tracker layers onto the Mapbox style.
 * Called from SatelliteLayers on style load and on data changes.
 *
 * iOS ref: SatelliteTrackLayer.swift
 */
fun renderTrackerLayers(
    style: Style,
    tracks: List<SatelliteTrack>,
    selectedId: String?
) {
    Log.d(TAG, "Rendering tracker layers: ${tracks.size} tracks, selected=$selectedId")
...
internal fun GeoJSONPolygon.toMapboxFeature(id: String): Feature? {
...
internal fun addOrUpdateSource(style: Style, sourceId: String, features: List<Feature>) {
...
internal fun addFillLayerIfNeeded(
    style: Style, layerId: String, sourceId: String,
...
internal fun addLineLayerIfNeeded(
    style: Style, layerId: String, sourceId: String,
...
internal fun addSymbolLayerIfNeeded(
    style: Style, layerId: String, sourceId: String,
...
internal fun addCircleLayerIfNeeded(
    style: Style, layerId: String, sourceId: String,
    block: com.mapbox.maps.extension.style.layers.generated.CircleLayerDsl.() -> Unit
) {
    if (!style.styleLayerExists(layerId)) {
        style.addLayer(com.mapbox.maps.extension.style.layers.generated.circleLayer(layerId, sourceId, block))
    }
}
